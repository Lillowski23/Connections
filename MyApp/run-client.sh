...
if [[ ! -d "$OUT_DIR" ]]; then
  ...
  needs_rebuild=true
elif [[ -n "$(find src -type f -name "*.java" -newer "$OUT_DIR/client/ClientMain.class" -print -quit 2>/dev/null)" ]]; then
  needs_rebuild=true
elif [[ "$GSON_JAR" -nt "$OUT_DIR/client/ClientMain.class" ]]; then
  needs_rebuild=true
fi

if [[ "$needs_rebuild" == true ]]; then
  echo "[run-client] out mancante/vuota/incompleta: compilo tutto src/..."
  mkdir -p "$OUT_DIR"
...
echo "[run-client] Avvio client.ClientMain"
